...
traps = read_csv("lobster_traps (1).csv") %>% 
  filter(TRAPS !="0", SITE == "AQUE"| SITE == "NAPL"| SITE == "MOHK" | SITE == "IVEE"| SITE == "CARP") # filtered out zero values and selected sites for report
...
```{r}
size_frequency = read.csv("lobster_size_abundance.csv", header = TRUE) %>% #tidy data = size_frequency file
