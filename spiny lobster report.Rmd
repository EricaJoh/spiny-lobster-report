...
ggplot(summary_size_2017b, aes(x = SITE, y = Mean))+
  ...
  labs(x = "SBC LTER Site", title = "SBC LTER mean lobster carapace length, 2017", y = "Mean carapace length & standard deviation (mm)")+
...
ggplot(size_2012_2017_non, aes(x=SIZE))+
  ...
  labs(x = "Carapace length (mm)", y= "Frequency (n)", title = "SBC LTER non-MPA sites lobster carapace length (mm), 2012 & 2017")+
  facet_wrap(~YEAR)+
  theme_classic()

ggplot(size_2012_2017_mpa, aes(x=SIZE))+
  geom_histogram(aes(col =SITE))+
  labs(x = "Carapace length (mm)", y= "Frequency (n)", title = "SBC LTER MPA sites lobster carapace length (mm), 2012 & 2017")+
  facet_wrap(~YEAR)+
  theme_classic()
...
#Chi-squared test
chi_test = chisq.test(lobster_prop_table, simulate.p.value = TRUE)
chi_test
...
#this is just a nice table to present. I tried presenting the proportions table but am unable to get site names on there!
lobster_legal_table2 = lobster_legal %>% 
  count(SITE, size) %>% 
  spread(size, n) 

kable(lobster_legal_table2, caption ="LTER SBC legal and sub-legal lobster size frequency by site in 2017")%>% 
    kable_styling(bootstrap_option = c("striped", "hover"), full_width = FALSE)
